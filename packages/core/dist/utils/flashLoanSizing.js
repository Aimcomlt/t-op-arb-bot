 ...
 */
export function computeSafeFlashLoanSize({ buyPool, sellPool, maxSlippageBps, }) {
    ...
    const withinSlippage = (dx) => {
        ...
        if (lhsBuy > rhsBuy)
            return false; // buy pool slippage too high
        ...
        if (lhsSell > rhsSell)
            return false; // sell pool slippage too high
        return true;
    };
    while (low <= high) {
