 ...
 */
export function scanDiscrepancy(snapshots, opts = {}) {
    const { minLiquidity = 0, thresholdBps = 0 } = opts;
    if (!snapshots.length)
        return null;
    const filtered = snapshots.filter((s) => s.reserves[0] >= minLiquidity && s.reserves[1] >= minLiquidity);
    ...
    for (let i = 1; i < prices.length; i++) {
        if (prices[i].price < min.price)
            min = prices[i];
        if (prices[i].price > max.price)
            max = prices[i];
    }
    if (min.dex === max.dex)
        return null;
    const spread = computeSpread(min, max);
    if (!spread || spread.spreadBps < thresholdBps)
        return null;
